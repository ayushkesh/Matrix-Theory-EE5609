...
If $\vec{a}$ = \myvec{ 5 \\ -1 \\ -3 } and $\vec{b}$= \myvec{ 1 \\ 3 \\ -5}, then show that the vectors $\vec{a+b}$ ans $\vec{a-b}$ are perpendicular.
...
\vec{A}\vec{B} = 0
\end{align}
Scalar product of two vectors, $\vec{A}$= \myvec{ x_1 \\ y_1 \\ z_1 } and $\vec{B}$= \myvec{ x_2 \\ y_2 \\ z_2 } is defined by
...
Let $\vec{a}$=\myvec{ 5 \\ -1 \\ -3 }, $\vec{b}$=\myvec{ 1 \\ 3 \\ -5}, and $\vec{A}$=$\vec{a+b}$ and $\vec{B}$=$\vec{a-b}$.
\begin{align}
\vec{A} &= \vec{a}+\vec{b} \\
\implies\vec{A} & = \myvec{ 5 \\ -1 \\ -3 } + \myvec{ 1 \\ 3 \\ -5} \\
\implies\vec{A} &= \myvec{ 6 \\ 2 \\ -8}
\end{align}
\begin{align}
\vec{B} &= \vec{a}-\vec{b} \\
\implies\vec{B} & = \myvec{ 5 \\ -1 \\ -3 } - \myvec{ 1 \\ 3 \\ -5} \\
\implies\vec{B} &= \myvec{ 4 \\ -4 \\ 2}
\end{align}
...
\end{align}
\begin{align}
\vec{A}\vec{B}= \brak{\myvec{ 5 \\ -1 \\ -3 } + \myvec{ 1 \\ 3 \\ -5}}^T\brak{\myvec{ 5 \\ -1 \\ -3 } - \myvec{ 1 \\ 3 \\ -5}}
\end{align}
\begin{align}
\vec{A}\vec{B}=\myvec{6 \\ 2\\ -8}^T\myvec{4\\-4\\-2}
\end{align}
\begin{align}
\implies\vec{A}\vec{B} &= \myvec{ 6 & 2 & -8}\myvec{ 4 \\ -4 \\ -2}
\end{align}
\begin{align}
\implies 6\times4+2\times(-4)+(-8)\times(-2) 
\end{align}
\begin{align}
\implies\vec{A}\vec{B} &= 24-8-16\\
